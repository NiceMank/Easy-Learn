/* exocheck.js — valide les 85 exercices : présence par langage, champs requis,
   répartition kind (30 dom / 55 checklist), et rendu réel d'une page exercice. */
...
async function main() {
  ...
  console.log('Exercices par langage :', JSON.stringify(perLang));
  console.log('TOTAL =', total, '(attendu', EXPECT_TOTAL + ') | dom =', dom_, '(attendu', EXPECT_DOM + ') | checklist =', check, '(attendu', EXPECT_CHECK + ')');
  if (total !== EXPECT_TOTAL) fails.push('total exercices ' + total + ' ≠ ' + EXPECT_TOTAL);
  if (dom_ !== EXPECT_DOM) fails.push('kind dom ' + dom_ + ' ≠ ' + EXPECT_DOM);
  if (check !== EXPECT_CHECK) fails.push('kind checklist ' + check + ' ≠ ' + EXPECT_CHECK);

  // Rendu réel des pages exercices (sommaire + une page d'exercice dom)
  await new Promise((r) => setTimeout(r, 150));
  const view = () => w.document.querySelector('#view');
  w.location.hash = '/exercices';
  await new Promise((r) => setTimeout(r, 150));
  if (!view().innerHTML.trim()) fails.push('page #/exercices vide');
  const firstDom = Object.keys(EXO).map((l) => ({ l, x: EXO[l].list.find((e) => e.kind === 'dom') })).find((p) => p.x);
  if (firstDom) {
    w.location.hash = '/exercices/' + firstDom.l + '/' + firstDom.x.id;
    await new Promise((r) => setTimeout(r, 200));
    const html = view().innerHTML;
    if (!html.trim()) fails.push('page exercice ' + firstDom.x.id + ' vide');
    else console.log('Rendu exercice «', firstDom.x.title.slice(0, 50), '» :', html.length, 'chars HTML');
  }
