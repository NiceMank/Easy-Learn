...
(function () {
  ...
  /** Noyau commun utilisé par JS, TS, et assimilés.
   *  Colorise les commentaires, chaînes (simples/doubles/template),
   *  mots-clés, appels de fonction, builtins, et nombres. */
  ...
  /* ----- JSX (post-traitement pour blocs js/ts/jsx) ----------------
     Après la coloration JS standard, les balises JSX apparaissent
     encore sous forme échappée (&lt;div&gt;). On les recolore a
     posteriori en appliquant la coloration HTML sur les portions
     qui ressemblent a du JSX — mais UNIQUEMENT hors des chaînes
     déjà colorées (tk-str).
     Prend du code DÉJÀ COLORÉ et ajoute la couche JSX par-dessus. */
  function applyJSX(alreadyColored) {
    // Découper en segments : hors tk-str / dans tk-str
    var parts = alreadyColored.split(/(<span class="tk-str">[^<]*(?:<(?!\/span>)[^<]*)*<\/span>)/g);
    for (var p = 0; p < parts.length; p++) {
      if (p % 2 === 0) {
        parts[p] = parts[p].replace(
          /(&lt;)(\/?)([A-Z][A-Za-z0-9]*(?:\.[A-Z][A-Za-z0-9]*)*|(?:[a-z][a-z0-9]*-)*[a-z][a-z0-9]*)([\s\S]*?)(\/?&gt;)/g,
          function (m, open, slash, tag, attrs, close) {
            // Filtrer : ne garder que ce qui ressemble à du JSX
            var jsxLike = /[A-Z]/.test(tag) ||
                          /^[a-z]+-[a-z]/.test(tag) ||
                          /^(div|span|p|a|img|ul|ol|li|h[1-6]|section|article|nav|header|footer|main|aside|button|input|form|label|select|option|textarea|table|tr|td|th|thead|tbody|br|hr|strong|em|code|pre|Fragment|Suspense|KeepAlive|Transition|TransitionGroup|RouterLink|RouterView|Teleport|component|slot|template)$/.test(tag);
            if (!jsxLike) return m;

            var coloredAttrs = attrs.replace(
              /([A-Za-z][\w-]*)(=)((?:"[^"]*")|(?:'[^']*')|(?:\{[^}]*\})|(?:[^\s"'{>&]+))/g,
              function (am, attr, eq, val) {
                return '<span class="tk-attr">' + attr + '</span>' + eq + '<span class="tk-str">' + val + '</span>';
              }
            );

            return '<span class="tk-tag">' + open + slash + tag + '</span>' + coloredAttrs + '<span class="tk-tag">' + close + '</span>';
          }
        );
      }
    }
  ...
  window.High = {
    ...
    run: function (code, lang) {
      ...
      if (lang === 'js' || lang === 'javascript')   return applyJSX(highlightJS(escaped));
      if (lang === 'ts' || lang === 'typescript')   return applyJSX(highlightTS(escaped));
      if (lang === 'jsx')                           return applyJSX(highlightJS(escaped));
