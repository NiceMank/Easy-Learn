...
'mon-app/\n├── index.html          # la COQUILLE HTML — c\'est elle, la vraie entrée\n├── package.json        # dépendances + scripts (dev/build/preview)\n├── vite.config.js      # config Vite (plugins, alias…)\n├── public/             # fichiers servis TELS QUELS (favicon, robots.txt)\n└── src/\n    ├── main.js         # createApp(App).use(router).mount("#app")\n    ├── App.vue         # composant racine\n    ├── assets/         # images/css IMPORTÉS (hashés, optimisés au build)\n    ├── components/     # tes composants\n    └── router/         # présent si tu as coché Vue Router' },
            ...
            { t: 'p', h: 'Lance `npm run dev`, modifie un composant, sauvegarde : la page se met à jour **sans rechargement complet et sans perdre l\'état**. C\'est le HMR : Vite ne remplace que le module modifié. Tu vas vite considérer ça comme normal — profites-en pour itérer par petits pas : modifier, regarder, corriger.' },
...
'<template>\n  <p v-if="score >= 90">Excellent !</p>\n  <p v-else-if="score >= 50">Encourageant…</p>\n  <p v-else>On révise et on réessaie.</p>\n\n  <!-- v-if sur un GROUPE sans balise en trop : <template> disparaît au rendu -->\n  <template v-if="connecte">\n    <h2>Tableau de bord</h2>\n    <p>Bienvenue {{ user.nom }}</p>\n  </template>\n</template>' },
            { t: 'p', h: '`v-else` et `v-else-if` doivent **immédiatement suivre** leur `v-if` : un élément intercalé casse la chaîne (le compilateur proteste). Et pour conditionner un *groupe* d\'éléments sans ajouter de balise parasite, on met `v-if` sur un `<template>` : il disparaît au rendu, seuls ses enfants conditionnés restent.' },
...
'<!-- Objet : la classe est présente SI la valeur est vraie -->\n<div :class="{ actif: estActif, \'texte-erreur\': erreur }"></div>\n\n<!-- Tableau : additionne des classes calculées -->\n<div :class="[base, estActif ? \'actif\' : \'\']"></div>\n\n<!-- Les deux se marient, et fusionnent avec le class statique -->\n<div class="carte" :class="[{ selectionne: id === choix }, variante]"></div>\n\n<!-- Style : objet JS, propriétés en camelCase -->\n<button :style="{ color: couleur, fontSize: taille + \'px\' }">OK</button>' },
...

        {
          ...
          intro: 'Côté interactions, Vue déclare les écouteurs **dans le template** avec `v-on:` — raccourci `@`, omniprésent. Là où le JavaScript vanilla impose `addEventListener` plus la gestion manuelle du retrait (fiche Événements du module JS), la directive s\'occupe de tout : attachement, retrait à la destruction du composant, contexte correct. Cerise sur le gâteau : les **modificateurs** (`.prevent`, `.stop`, `.enter`, `.once`) remplacent les incantations `event.preventDefault()` éparpillées dans le code.',
          blocks: [
...
'<form @submit.prevent="envoyer">…</form>   <!-- empêche le rechargement -->\n<a @click.stop="ouvrir">…</a>              <!-- stoppe la propagation -->\n<button @click.once="activer">…</button>   <!-- une seule fois -->\n<input @keyup.enter="valider">             <!-- uniquement la touche Entrée -->\n<div @click.self="fermer">…</div>          <!-- clic sur l\'élément même, pas un enfant -->\n<button @click.left.stop="action">…</button>   <!-- les modificateurs se chaînent -->' },
...

        {
          ...
          intro: 'Par défaut, Vue **échappe** tout : `{{ "<b>x</b>" }}` affiche littéralement les balises. `v-html` court-circuite cette protection et injecte la chaîne comme HTML réellement interprété. Usage légitime : du contenu riche venant d\'un CMS, ou de TON Markdown compilé. Usage catastrophique : afficher une saisie utilisateur — commentaire, pseudo, message — qui peut embarquer un script ou un gestionnaire inline malveillant. C\'est la porte d\'entrée classique du **XSS** (injection de script).',
          blocks: [
...
'import { ref, computed, onMounted } from "vue";\n\nexport default {\n  setup() {\n    const compteur = ref(0);                          // l\'état\n    const double = computed(() => compteur.value * 2); // les dérivés\n    function incrementer() { compteur.value++; }       // les actions\n    onMounted(() => console.log("monté !"));           // le cycle de vie\n\n    return { compteur, double, incrementer };   // exposé au template\n  }\n};' },
...

        {
          ...
          intro: '`<script setup>` est la forme que tu écriras 95 % du temps : même puissance que `setup()`, sans la fonction ni le `return`. Tout ce qui est déclaré au niveau supérieur — variables, imports, fonctions — est directement utilisable dans le template. Les déclarations spéciales (`defineProps`, `defineEmits`, `defineModel`, `defineExpose`) sont des **macros de compilation** : tu les appelles sans les importer, et elles disparaissent au build.',
          blocks: [
            { t: 'h3', h: 'Concrètement' },
...
'<template>\n  <input v-model="terme" placeholder="Rechercher…">\n  <p>{{ resultats.length }} résultat(s)</p>\n  <CarteResultat v-for="r in resultats" :key="r.id" :resultat="r" />\n</template>\n\n<script setup>\n// Tout ce niveau supérieur est exposé au template, sans return :\nimport { ref, computed } from "vue";\nimport CarteResultat from "./CarteResultat.vue";   // importé = utilisable dans le template\n\nconst props = defineProps({ source: Array });      // macro : s\'utilise telle quelle\nconst emit = defineEmits(["selection"]);\n\nconst terme = ref("");\nconst resultats = computed(() =>\n  (props.source ?? []).filter((x) => x.titre.includes(terme.value))\n);\n<\/script>' },
            ...
            { t: 'ul', items: [
              ...
              'Le `await` au niveau supérieur est possible, mais le composant devient alors asynchrone : il doit être enveloppé dans `<Suspense>`. À réserver aux cas qui le méritent.',
      ...
      fiches: [
        {
          ...
          intro: 'Un composant utile est **paramétrable** : la carte affiche LE produit qu\'on lui passe. Ce paramétrage, ce sont les props — les "arguments" du composant. Vue les déclare explicitement avec `defineProps`, les type, les valide et les dote de valeurs par défaut. Principe non négociable : le flux est **descendant**. L\'enfant lit ses props, ne les modifie JAMAIS ; s\'il veut signaler quelque chose, il émet un événement (fiche suivante).',
          blocks: [
...
'const props = withDefaults(defineProps({\n  titre: String,\n  etapes: Array\n}), {\n  titre: "Sans titre",\n  etapes: () => []\n});' },
...

        {
          ...
          intro: 'Les props descendent, les **événements remontent** : c\'est l\'autre moitié de la communication parent-enfant. Un bouton ne sait pas ce que son clic signifie pour le métier ; il annonce `emit("ajouter", article)` et le parent décide. `defineEmits` déclare ces événements (avec validation possible), le parent écoute avec `@ajouter="..."` — exactement comme un événement natif.',
          blocks: [
      ...
      fiches: [
        {
          ...
          intro: 'Un composant naît, s\'insère dans le DOM, se met à jour, puis disparaît. À chaque étape correspond un **hook de cycle de vie** qu\'on importe : `onMounted`, `onUpdated`, `onUnmounted`… Savoir quel hook choisir évite 80 % des bugs "ça marche une fois sur deux" : DOM pas encore là, timers qui survivent au composant, double abonnement. Une règle d\'hygiène en découle : tout ce qui s\'ouvre quelque part se ferme dans `onUnmounted`.',
          blocks: [
      ...
      fiches: [
        {
          ...
          intro: 'Tout ce qu\'on a vu (props, emits, slots à venir) sert un dessein : découper l\'interface en **briques réutilisables** qu\'on assemble. La règle de découpe n\'est pas « un fichier par écran » mais « une responsabilité par composant » : quand un fichier dépasse ~200 lignes ou mélange deux sujets, il demande à être scindé. Cette fiche couvre l\'assemblage : enregistrement, héritage des attributs, et le couteau suisse `<component :is>`.',
          blocks: [
      ...
      fiches: [
        {
          ...
          intro: 'Tant que la donnée voyage parent → enfant, props et events suffisent. Quand elle doit circuler entre des branches éloignées — le compteur du panier dans l\'en-tête, son contenu dans une autre page — passer par dix niveaux devient du bruit : c\'est le "prop drilling". **Pinia**, le store officiel, offre des entrepôts réactifs accessibles de partout : `defineStore` décrit un store (état, calculés, actions), `useXStore()` le consomme. Principe gardien : **un store n\'est pas une poubelle** — l\'état local reste la norme, le store l\'exception.',
          blocks: [
      ...
      fiches: [
        {
          ...
          intro: 'Vue ne fournit pas de client HTTP : on utilise `fetch` (natif) ou axios. La vraie compétence n\'est pas l\'appel, c\'est l\'**orchestre** : où déclencher (onMounted ou un watcher), comment représenter les trois états (données / chargement / erreur), et comment éviter les courses quand l\'utilisateur clique plus vite que le réseau. C\'est exactement le sujet que le module TanStack Query approfondit — et les mêmes concepts s\'appliquent ici (vue-query existe aussi).',
          blocks: [
      ...
      fiches: [
        {
          ...
          intro: 'Quand deux composants ont besoin de la MÊME logique réactive — suivre la souris, lire localStorage, paginer une API — ni les props ni les mixins (hérités de Vue 2, à fuir) n\'offrent une réponse propre. Le **composable** est une simple fonction qui utilise l\'API de Composition (`ref`, `computed`, `watch`, hooks de cycle) et retourne ce qui sert — par convention des refs, nommée `useQuelqueChose`. C\'est l\'analogue des hooks custom de React, en plus libre : pas de règle d\'ordre d\'appel, `setup` ne s\'exécutant qu\'une fois.',
          blocks: [
