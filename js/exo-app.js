/* ============================================================
   exo-app.js — Module « Exercices Pratiques » (Premium)
   ------------------------------------------------------------
   Vues : hub (#/exercices), page module (#/exercices/<mod>),
   détail (#/exercices/<mod>/<exo>), paywall (#/exercices/premium).
...
(function () {
  ...
  function markDone(e) {
    ...
    toast('Exercice réussi : ' + e.title, 'task_alt');
  }
  ...
  function promoBanner() {
    return '<div class="promo-banner view-anim">' +
      ...
      '<span>Tous les exercices, tous les modules : ateliers de code, indices progressifs, solutions commentées et badges de progression.</span></span>' +
      '<a class="btn" href="#/exercices/premium">' + icon('lock_open') + '<span>Débloquer Premium</span></a></div>';
  }
  ...
  function modCard(m) {
    ...
    return '<a class="exo-mod-card" data-exo-mod-card data-name="' + esc((L.name + ' ' + m).toLowerCase()) + '"' +
      ' style="--mc:var(--c-' + m + ')" href="#/exercices/' + m + '">' +
      ...
      '<span class="mod-sub">' + s.total + ' exercices · 3 niveaux</span></span>' +
      ...
      (Premium.isUnlocked() ? '' : '<span class="free-chip" title="Exercice(s) accessible(s) sans Premium">' + icon('lock_open') + freeCount + ' gratuit</span>') +
      '</span></a>';
  }
  ...
  function hubView() {
    ...
    /* Héro premium sombre — l'écran d'accueil des exercices */
    var html = '<section class="exo-hero view-anim">' +
      ...
      '<span class="exo-hero-eyebrow">' + icon('military_tech') + ' Exercices pratiques</span>' +
      '<h1>Prouve ta maîtrise<br>en codant.</h1>' +
      '<p class="lead">La documentation t\'a appris les notions : ici, tu les mets en œuvre. Des exercices progressifs — Fondamentaux, Intermédiaires, Projets réels — adossés à chaque module de la doc.</p>' +
      '<div class="exo-hero-stats">' +
      '<span class="exo-hero-stat">' + icon('format_list_numbered') + '<span><strong>' + tot + '</strong> exercices</span></span>' +
    ...
    html += '<section class="exo-progress view-anim">' +
      ...
      '<span>' + done + ' exercice' + (done > 1 ? 's' : '') + ' réussi' + (done > 1 ? 's' : '') + ' sur ' + tot +
    ...
    if (resume) {
      html += '<a class="list-row view-anim" href="#/exercices/' + mid + '/' + resume.id + '" style="margin-bottom:26px">' +
        '<span class="card-icon ' + ICON_CLS[mid] + '">' + icon('resume') + '</span>' +
        '<span class="list-row-text"><span class="list-row-sub">Reprendre ton exercice en cours</span>' +
    ...
    GROUPS.forEach(function (g) {
      ...
      html += '<div class="exo-group" data-exo-group>' +
        ...
        '<span class="exo-group-sub">' + gmods.length + ' modules · ' + gTot + ' exercices' + (gDone ? ' · ' + gDone + ' réussis' : '') + '</span></h3>' +
  ...
  function exoCard(m, e) {
    ...
    return '<a class="exo-card ' + st + '" data-level="' + e.level + '" style="--lc:var(--lv-' + e.level + ')" href="#/exercices/' + m + '/' + e.id + '">' +
  ...
  function moduleView(m) {
    ...
    var html = '<nav class="breadcrumb view-anim" aria-label="Fil d\'Ariane"><a href="#/exercices">Exercices</a>' + icon('chevron_right') + '<span>' + esc(L.name) + '</span></nav>';
    ...
    html += '<header class="exo-mod-hero view-anim" style="--mc:var(--c-' + m + ')">' +
      ...
      '<div class="mh-text"><h1>Exercices ' + esc(L.name) + '</h1>' +
      ...
      '<div class="mh-chips">' +
      '<span class="mh-chip">' + icon('format_list_numbered') + s.total + ' exercices</span>' +
  ...
  function checklistHtml(e) {
    ...
    html += '<div class="check-panel-foot"><span class="note">' + icon('terminal') +
      ' Exécute ton programme en local, teste réellement chaque point, puis coche-le. Quand tout est vert, valide l\'exercice.</span>' +
      '<button type="button" class="btn small" data-complete' + (done === n ? '' : ' disabled') + '>' + icon('flag') + '<span>J\'ai terminé cet exercice</span></button></div></div>';
    return html;
  }
  ...
  function lockedZone(e) {
    ...
    return '<div class="locked-zone view-anim"><div class="locked-blur" aria-hidden="true">' + skel + '</div>' +
      ...
      '<h3>Exercice réservé à Premium</h3>' +
      ...
      '<a class="btn" href="#/exercices/premium">' + icon('lock_open') + '<span>Débloquer Premium</span></a>' +
      '<p style="margin:12px 0 0;font-size:12px">Chaque module offre 1 exercice Fondamentaux gratuit.</p>' +
      '</div></div></div>';
  }

  function detailView(m, e) {
    var L = DEVDOCS[m], acc = accessible(e), st = statusOf(e);
    var html = '<nav class="breadcrumb view-anim" aria-label="Fil d\'Ariane"><a href="#/exercices">Exercices</a>' + icon('chevron_right') +
      '<a href="#/exercices/' + m + '">' + esc(L.name) + '</a>' + icon('chevron_right') + '<span>' + esc(e.title) + '</span></nav>';
  ...
  function paywallView() {
    ...
    if (premium) {
      return '<div class="paywall-card view-anim">' +
        ...
        '<a class="btn" href="#/exercices">' + icon('military_tech') + '<span>Retour aux exercices</span></a>' +
    ...
    return '<div class="paywall-card view-anim">' +
      ...
      '<p>Tous les exercices de tous les modules, pour transformer la théorie en réflexes de développeur. <strong>9 900 FCFA / an</strong> — tarif de lancement (simulé pour cette démonstration).</p>' +
      '<div class="paywall-benefits">' +
      '<span class="li" style="display:contents"></span>' +
      '<li>' + icon('task_alt') + '<span><strong>85 exercices progressifs</strong><span class="sub">Fondamentaux → Intermédiaires → Projets réels, dans les 17 modules de la doc.</span></span></li>' +
  ...
  function route(parts) {
    ...
    if (!mod) {
      ...
      return { html: hubView(), title: 'Exercices pratiques' };
    }
    if (mod === 'premium') {
    ...
    if (window.DEVDOCS && DEVDOCS[mod] && X()[mod]) {
      ...
      if (!id) {
        ...
        return { html: moduleView(mod), title: 'Exercices ' + DEVDOCS[mod].name };
      }
    ...
    if (t && current.exo && !t.disabled) {
      ...
      if (panel && !panel.parentElement.querySelector('.success-pop')) {
        panel.insertAdjacentHTML('afterend',
          ...
          '<span><strong>Exercice validé — beau travail !</strong>' +
  ...
  function onSubmit(ev) {
    ...
    toast('Premium activé — tous les exercices sont ouverts', 'workspace_premium');
    location.hash = lastLocked || '#/exercices';
    /* Si déjà sur la bonne route (hash inchangé), force le re-rendu */
    if ((lastLocked || '#/exercices') === location.hash) rerender();
  }
